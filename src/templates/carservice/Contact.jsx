...
export default function CarServiceContact() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSending(true)
    try {
      await submitForm('carservice_request', form)
      setSubmitted(true)
    } catch (err) {
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="grid gap-5">
              ...
              <button type="submit" disabled={sending} className="mt-2 rounded-md bg-[#FF6B35] px-6 py-3.5 font-sans text-sm font-semibold text-white transition-transform hover:scale-[1.02] disabled:opacity-60">
