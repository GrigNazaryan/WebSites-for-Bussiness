...
export default function BarbershopContact() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSending(true)
    try {
      await submitForm('barbershop_booking', form)
      setSubmitted(true)
    } catch (err) {
        ...
        <div className="border border-white/10 bg-[#0A0A0A] p-8">
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="grid gap-5">
              ...
              <button type="submit" disabled={sending} className="mt-2 bg-[#F5A623] px-6 py-3.5 font-sans text-sm font-bold uppercase tracking-wide text-black transition-transform hover:scale-[1.02] disabled:opacity-60">
