...
export default function RestaurantContact() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSending(true)
    try {
      await submitForm('restaurant_reservation', form)
      setSubmitted(true)
    } catch (err) {
        ...
        <div className="rounded-3xl border border-white/10 bg-[#14100D] p-8">
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="grid gap-5">
              ...
              <button type="submit" disabled={sending} className="mt-2 rounded-full bg-[#C9A24B] px-6 py-3.5 font-sans text-sm font-semibold text-[#14100D] transition-transform hover:scale-[1.02] disabled:opacity-60">
